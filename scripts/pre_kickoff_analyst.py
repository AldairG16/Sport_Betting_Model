...
# ── Diagnóstico de fallos tolerados ───────────────────────────────────────────
# Contrato de visibilidad: en la ruta del LLM un `except` ancho puede mantener
# viva la corrida, pero nunca tragarse el error. Un `except Exception: pass`
# alrededor de la llamada al analista ya costó un día entero de fallos
# silenciosos, indistinguibles de un día sin value bets.
# Silencio = salud: no imprime nada en el camino feliz.
def _warn(context: str, exc: Exception) -> None:
    """Deja rastro de un fallo tolerado, sin abortar la corrida."""
    print(f"[ANALYST] {context}: {type(exc).__name__}: {exc}")
...
def _load_analyzed_set(bets_df: pd.DataFrame) -> set:
    ...
    try:
        ...
        # Sin dedupe se puede re-analizar un partido ya visto: caro, no fatal.
        _warn("No se pudo leer pre_kickoff_analyses para el dedupe", e)
        return set()
...
def _load_manual_lessons() -> str:
    ...
    try:
        ...
        # Sin lecciones el prompt queda más pobre y el dictamen cambia:
        # eso tiene que verse, no degradarse en silencio.
        _warn(f"No se pudo leer {LESSONS_FILE.name}", e)
    return ""
...
def _build_learning_memo(days: int = 30, max_examples: int = 3) -> str:
    ...
    try:
        ...
        # Tabla aún no migrada o error de DB → memo vacío, pero visible.
        _warn("No se pudo construir el memo de aprendizaje", e)
        return ""
...
def _build_context(bet: dict) -> dict:
    ...
    # Cada pieza del dosier es opcional: si una falla seguimos con el resto
    # (el agente tolera un dosier incompleto), pero el fallo se reporta. Un
    # dosier a medias cambia el dictamen y antes se veía igual que uno completo.
    try:
        ctx["h2h"] = get_h2h_stats(home, away, cutoff_date=md)
    except Exception as e:
        _warn(f"h2h no disponible para {home} vs {away}", e)
        ctx["h2h"] = None

    try:
        ctx["form_home"] = get_team_form(home, venue="home", cutoff_date=md)
    except Exception as e:
        _warn(f"Forma local no disponible para {home}", e)
        ctx["form_home"] = None

    try:
        ctx["form_away"] = get_team_form(away, venue="away", cutoff_date=md)
    except Exception as e:
        _warn(f"Forma visitante no disponible para {away}", e)
        ctx["form_away"] = None
    ...
    try:
        ...
        _warn(f"Congestión no disponible para {home}", e)
        ctx["cong_home"] = None
        ctx["cong_home_n"] = 0
    ...
    try:
        ...
        _warn(f"Congestión no disponible para {away}", e)
        ctx["cong_away"] = None
        ctx["cong_away_n"] = 0

    try:
        ctx["line"] = get_line_movement(match_key)
    except Exception as e:
        _warn(f"Movimiento de línea no disponible para {match_key}", e)
        ctx["line"] = None

    try:
        ctx["motiv_home"] = float(get_motivation_factor(home, league))
    except Exception as e:
        _warn(f"Motivación no disponible para {home}", e)
        ctx["motiv_home"] = 0.0

    try:
        ctx["motiv_away"] = float(get_motivation_factor(away, league))
    except Exception as e:
        _warn(f"Motivación no disponible para {away}", e)
        ctx["motiv_away"] = 0.0
    ...
    try:
        ...
        _warn(f"xG no disponible para {home}", e)
    ...
    try:
        ...
        _warn(f"xG no disponible para {away}", e)
        ctx["xg_away_for"] = ctx["xg_away_against"] = None

    try:
        ctx["weather"] = float(get_weather_multiplier(home))
    except Exception as e:
        _warn(f"Clima no disponible para {home}", e)
        ctx["weather"] = 1.0
...
def _normalize_result(parsed: dict, bet: dict) -> dict:
    ...
    try:
        ...
        # Odds mal formadas: dejamos el dictamen del agente tal cual, pero un
        # cross-check cuantitativo que no corrió no puede ser invisible.
        _warn("No se pudo aplicar el cross-check de edge al dictamen", e)
...
def debug_mode_run():
    ...
    try:
        ...
        try:
            ...
            _warn("No se pudo enviar el aviso de DEBUG SANITY CHECK FAILED", e)
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            ...
            # El heartbeat es el rastro del crash; si ni eso se puede escribir
            # hay que verlo en stdout antes de que el proceso muera.
            _warn("No se pudo escribir el heartbeat del crash", e)
