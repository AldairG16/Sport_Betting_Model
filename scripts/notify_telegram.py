...
# Contrato de visibilidad: un `except` ancho en la ruta de notificación puede
# mantener viva la corrida — el mensaje del día vale más que un campo suelto —
# pero nunca puede tragarse el error. Éste es el único lugar donde se decide
# el formato del rastro. Silencio = salud: no imprime nada en el camino feliz.
def _warn(context: str, exc: Exception) -> None:
    """Deja rastro de un fallo tolerado, sin abortar el mensaje."""
    print(f"[NOTIFY] {context}: {type(exc).__name__}: {exc}")
...
def _format_bet_line(i: int, bet, suspicious: bool) -> str:
    ...
    if "match_date" in bet and bet["match_date"]:
        try:
            ...
            _warn(f"Fecha no parseable en {bet.get('match', '?')}", e)
...
def _read_paper_trades_for_date(target_date, days_ahead: int = 0) -> list:
    ...
    for r in rows:
        ...
        try:
            ...
            _warn(f"Fecha no parseable ({md!r}); fila omitida", e)
            continue
    ...
    for league_key in sorted(by_league.keys()):
        ...
        for r in by_league[league_key]:
            ...
            try:
                ...
                _warn(f"edge no numérico en {r.get('match', '?')}", e)
                edge_pct = 0.0
            try:
                prob_pct = round(float(r.get("probability", 0)) * 100, 1)
            except Exception as e:
                _warn(f"probability no numérica en {r.get('match', '?')}", e)
                prob_pct = 0.0
            try:
                odds_val = round(float(r.get("odds", 0)), 2)
            except Exception as e:
                _warn(f"odds no numéricas en {r.get('match', '?')}", e)
                odds_val = 0.0
            try:
                stake_val = round(float(r.get("stake", 0)), 2)
            except Exception as e:
                _warn(f"stake no numérico en {r.get('match', '?')}", e)
                stake_val = 0.0
            ...
            if md:
                try:
                    ...
                    _warn(f"Fecha no parseable ({md!r})", e)
...
def _format_kickoff_label(match_date) -> str:
    ...
    try:
        ...
        _warn(f"Fecha de partido no parseable ({match_date!r})", e)
        return ""
    today = _local_today()
    tomorrow = _local_tomorrow()
    d = dt_local.date()
...
def send_health_check(
    ...
    try:
        ...
        _warn("No se pudo leer el log de créditos de la API", e)
    ...
    try:
        ...
        _warn("Stats de bankroll no disponibles para el health check", e)
...
def send_weekly_report():
    ...
    try:
        ...
        # Fallback sin created_at (la columna puede no existir todavía).
        _warn("Query semanal por created_at falló; se usa match_date", e)
        week_df = pd.read_sql("""
            SELECT market, odds, stake, result, profit, league
            FROM bets_history
            WHERE result NOT IN ('pending')
    ...
    try:
        ...
        _warn("No se pudo leer el histórico completo de bets_history", e)
        all_df = pd.DataFrame()
    ...
    try:
        ...
        _warn("Bankroll no disponible; se usan valores por defecto", e)
...
def send_evening_summary():
    ...
    try:
        ...
        # La tabla puede no existir aún o la DB puede fallar → no rompemos el
        # resumen, pero el fallo queda en el log del run.
        _warn("No se pudo leer analyst_heartbeat para el resumen", e)
