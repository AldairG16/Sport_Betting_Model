...
def _drop_null_probability(df):
    """Separa las filas sin probability y devuelve (df_limpio, n_excluidas).

    Función pura sobre el DataFrame — el conteo es lo que hace auditable el
    descarte. Un salto de 0 a N en ese número es la señal de que algo
    upstream (migración, parseo del agente) se rompió.
    """
    if df.empty or "probability" not in df.columns:
        return df, 0
    keep = df["probability"].notna()
...
def main():
    ...
    # ── Descarte de probability NULL, contado ────────────────────────
    # Antes el WHERE traia un filtro NOT NULL sobre p.probability: las
    # filas sin probabilidad desaparecían sin dejar rastro, así que una
    # migración a medias o un parseo roto del agente se veía exactamente
    # igual que un período sin análisis. Ahora el filtro vive acá y el
    # descarte se reporta. Silencio = salud: solo hablamos si descartamos.
    df, excluded = _drop_null_probability(df)
    if excluded:
        print(f"⚠️  {excluded} fila(s) excluidas por probability NULL "
              f"— revisá la migración de la columna y el parseo del agente.\n")
