...
def _check_world_cup_activation():
    ...
    if WORLD_CUP_KEY not in _settings.SPORT_KEYS:
        ...
        try:
            ...
            # El aviso es informativo: si Telegram falla no abortamos la
            # activación del Mundial, pero el fallo queda en el log.
            print(f"[ORCHESTRATOR] No se pudo avisar la activación del Mundial: {e}")
        ...
        with _engine.begin() as conn:
            for stmt in ddl + indexes:
                try:
                    ...
                    # Normalmente el índice/tabla ya existe, o la tabla
                    # relacionada aún no fue creada. Toleramos el fallo pero
                    # dejamos rastro: un DDL que falla siempre es un bug.
                    _stmt = " ".join(stmt.split())[:70]
                    print(f"[ORCHESTRATOR] DDL omitido «{_stmt}»: {e}")
    except Exception as e:
        # No bloqueamos el pipeline por los índices, pero el fallo se ve.
        print(f"[ORCHESTRATOR] No se pudieron asegurar índices/tablas: {e}")
...
def _alert_step_failed(step_name: str):
    ...
    try:
        ...
        # Si ni la alerta se puede enviar, que al menos quede en el log
        # del run: el step fallido ya es invisible de por sí.
        print(f"[ORCHESTRATOR] No se pudo alertar el fallo de '{step_name}': {e}")
...
def main():
    ...
    try:
        ...
        try:
            ...
            # El crash original se re-lanza igual; esto sólo evita que el
            # fallo del canal de alerta se pierda.
            logger.log(f"[ORCHESTRATOR] No se pudo enviar el crash report: {e}")
        ...
        if args.mode in ("morning", "evening", "full"):
            try:
                ...
                # Nunca bloqueamos el finally por el health check, pero
                # tampoco lo perdemos: si esto falla el usuario no recibe
                # el resumen del ciclo y no se entera de por qué.
                logger.log(f"[ORCHESTRATOR] Health check no enviado: {e}")
