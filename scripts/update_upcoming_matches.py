...
# Todas las constantes que gatean comportamiento de producción viven en
# config/settings.py — una sola fuente de verdad. Este módulo las importa,
# nunca las declara: si un umbral se declara aquí, el workflow que no lo
# pasa en su bloque `env:` corre con un valor distinto al de los demás
# (ese fue el hueco de weekly.yml vs morning/closing/evening).
from config.settings import (
    ...
    API_CREDITS_STOP_THRESHOLD, MAX_CREDITS_PER_RUN, MAX_ENRICHMENTS_PER_RUN,
    ENRICH_DAYS_AHEAD, ENRICH_CACHE_TTL_HOURS,
)
...
# ENRICH_DAYS_AHEAD y ENRICH_CACHE_TTL_HOURS se importan de config.settings
# (ver bloque de imports arriba) — no se declaran aquí.
