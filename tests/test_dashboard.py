...
def _bet(id_, day, match, result, profit, odds=1.80, clv=None):
    return {"id": id_, "match_date": pd.Timestamp(f"2026-09-{day} 15:00", tz="UTC"),
            ...
            "profit": profit, "closing_odds": None, "clv": clv}
...
BETS = pd.DataFrame([
    _bet(1, 19, "alpha vs beta", "win", 0.80, clv=0.016),
    _bet(2, 20, "gamma vs delta", "loss", -1.00, clv=-0.015),
    ...
    def fake_q(sql, params=None):
        ...
        if "FROM goalscorer_picks" in s:
            return pd.DataFrame([{"match_date": "2026-09-21", "match": "alpha vs beta",
                                  ...
                                  "probability": 0.3, "fair_odds": 3.3, "result": "win"}])
...
def test_bets_show_prices_like_playdoit(client):
    """Formato americano (+128 / -140), como los muestra PlayDoit: el dueño
    no lee decimales. Mejor cuota 1.80 → -125; prob 0.60 → mínimo -138."""
    bets = client.get("/api/bets?status=all").get_json()["bets"]
    first = next(b for b in bets if b["id"] == 1)
    assert (first["odds_us"], first["min_us"]) == ("-125", "-138")
    assert first["min_hint"] == "Sirve -125; -150 ya no"
    assert "odds_placed" not in first and "min_odds" not in first
...
def test_nothing_to_type_by_hand(dash):
    """v1.5.0: el dueño no quiere pasos manuales — se quitó el registro de
    la cuota tomada en PlayDoit (v1.4.0). Las únicas rutas POST disparan
    workflows o el diagnóstico; ninguna escribe en tus apuestas."""
    posts = sorted(r.rule for r in dash.app.url_map.iter_rules() if "POST" in r.methods)
    assert posts == ["/api/gh/dispatch/<wf>", "/api/narrative/generate"]


def test_kpis_have_no_manual_entry_cards(client):
    k = client.get("/api/kpis").get_json()
    assert not {"placed_n", "real_roi", "slippage_avg"} & set(k)
...
def test_tables_and_panels(client):
    ...
    scorers = client.get("/api/scorers").get_json()
    assert scorers["ok"] and scorers["picks"][0]["fair_us"] == "+230"     # 3.30 → +230
