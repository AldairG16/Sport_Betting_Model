...
python scripts/orchestrator.py --mode evening   # 07:30 MX — results + CLV + nightly + preview
python scripts/orchestrator.py --mode results   # just re-fetch results and resolve bets
python scripts/orchestrator.py --mode weekly    # Mon 07:00 MX — reload historical + calibration + walk-forward
python scripts/clv_audit.py                      # second step of weekly.yml, continue-on-error

# The two LLM-driven agents (separate from the orchestrator, separate workflows)
python scripts/pre_kickoff_analyst.py            # cron 7,22,37,52 from pre_kickoff.yml (~every 15 min)
...
# Tests — tests/conftest.py bootstraps a DB-free env, so the suite runs
# with no .env locally and identically in CI.
...
# Static repository audit — no DB, no secrets, no API spend. Writes
# audits/latest.json + audits/latest.md. `--ci` runs the CI-gate subset.
...
Do not restate these values anywhere else — link to them instead. Every one of
them drifted at least once, and the static audit now fails on a re-divergence.

| Question | Authority |
|---|---|
| Default of any tunable constant | `config/settings.py` (read via `env_int` / `env_float` / `env_str`) |
| Tables and columns of the DB | `docs/schema.md` |
| What every file in `scripts/` is for, and whether it writes to the DB | `docs/scripts_inventory.md` |
| Run times and the env block of each loop | `.github/workflows/*.yml` |
| Disposition of every unreachable module | `archive/ARCHIVE.md` |
...
- `can_call(engine, estimated_cost_usd)` rejects the call if today's spend + estimate would exceed `ANTHROPIC_DAILY_BUDGET_USD` (default $0.20, override via GH secret).
